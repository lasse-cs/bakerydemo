...
class BreadPageFilterSet(PageFilterSet):
    bread_type = ModelChoiceFilter(queryset=BreadType.objects.all(), widget=RadioSelect)
    origin = ModelChoiceFilter(queryset=Country.objects.all(), widget=RadioSelect)
    ingredients = ModelMultipleChoiceFilter(
        queryset=BreadIngredient.objects.all(), widget=CheckboxSelectMultiple
    )
...
class BreadPageListingViewSet(PageListingViewSet):
    ...
    def columns(cls):
        ...
        return [
            col if col.name != "parent" else origin_column
            for col in PageListingViewSet.columns
        ]
